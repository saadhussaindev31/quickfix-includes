...
namespace FIX50
{
  ...
  {
    static FIX::MsgType::Pack PackedType() { return FIX::MsgType::Pack("AK"); }
  public:
    Confirmation() : Message(PackedType()) {}
    ...
    : Message(PackedType())
    {
      ...
      Sequence::push_back_to(*this, aAllocQty);
      Sequence::push_back_to(*this, aNetMoney);
      Sequence::push_back_to(*this, aGrossTradeAmt);
      Sequence::push_back_to(*this, aConfirmID);
      Sequence::push_back_to(*this, aConfirmStatus);
      Sequence::push_back_to(*this, aConfirmTransType);
      Sequence::push_back_to(*this, aConfirmType);
    }

    Confirmation(
      const FIX::ConfirmID::Pack& aConfirmID,
      const FIX::ConfirmTransType::Pack& aConfirmTransType,
      const FIX::ConfirmType::Pack& aConfirmType,
      const FIX::ConfirmStatus::Pack& aConfirmStatus,
      const FIX::TransactTime::Pack& aTransactTime,
      const FIX::TradeDate::Pack& aTradeDate,
      const FIX::AllocQty::Pack& aAllocQty,
      const FIX::Side::Pack& aSide,
      const FIX::AllocAccount::Pack& aAllocAccount,
      const FIX::AvgPx::Pack& aAvgPx,
      const FIX::GrossTradeAmt::Pack& aGrossTradeAmt,
      const FIX::NetMoney::Pack& aNetMoney )
    : Message(PackedType())
    {
      // must be in this order
      Sequence::push_back_to(*this, aAvgPx);
