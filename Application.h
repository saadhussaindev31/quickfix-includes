...
public:
  ...
  virtual void toApp( Message&, const SessionID& )
    THROW_DECL( DoNotSend ) = 0;
  /// Notification of admin message being received from target
  virtual void fromAdmin( const Message&, const SessionID& )
    THROW_DECL( FieldNotFound, IncorrectDataFormat, IncorrectTagValue, RejectLogon ) = 0;
  /// Notification of app message being received from target
  virtual void fromApp( const Message&, const SessionID& )
    THROW_DECL( FieldNotFound, IncorrectDataFormat, IncorrectTagValue, UnsupportedMessageType ) = 0;
};
...
public:
  ...
  void toApp( Message& message, const SessionID& sessionID )
    THROW_DECL( DoNotSend ) { Locker l( m_mutex ); app().toApp( message, sessionID ); }
